...
const renamePatterns = [
  ["var placeholder1 = expression1('placeholder2');", varNameFromModulePath],
  ["var placeholder1 = _interopRequireDefault(expression1('placeholder2'));", varNameFromModulePath],
  ["var placeholder1 = expression1('placeholder2').placeholder3();", varNameFromModulePath],
  ["const placeholder1 = expression1('placeholder2');", varNameFromModulePath],
  ["const placeholder1 = _interopRequireDefault(expression1('placeholder2'));", varNameFromModulePath],
  ["const placeholder1 = expression1('placeholder2').placeholder3();", varNameFromModulePath],
  ...
  ["var placeholder1 = new placeholder2(expression1_repeatable_optional);", varNameFromClassName],
  ["let placeholder1 = new placeholder2(expression1_repeatable_optional);", varNameFromClassName],
  ["const placeholder1 = new placeholder2(expression1_repeatable_optional);", varNameFromClassName],
...
function varNameFromClassName({placeholder1: oldName, placeholder2: className})
{
  let match = /^([A-Z]+)([a-z]\w*)$/.exec(className);
  if (!match)
    return null;

  return [oldName, match[1].toLowerCase() + match[2]];
}
...
export default function deduceVariableNames(ast)
{
  ...
  estraverse.traverse(ast, {
    enter(node)
    {
      ...
      let renaming = null;
      if (scopes[scopes.length - 1].node.type != "Program")
      {
