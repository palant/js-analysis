...
class StatementPlaceholder
{
  ...
  modifier(name)
  {
    ...
    {
      this.allowStatements = true;
      this.allowFunctionDeclarations = this.allowVariableDeclarations = false;
    }
    else if (name == "declaration")
    {
      this.allowStatements = false;
      this.allowFunctionDeclarations = this.allowVariableDeclarations = true;
    }
    else if (name == "functionDeclaration")
    {
      this.allowStatements = this.allowVariableDeclarations = false;
      this.allowFunctionDeclarations = true;
    }
    else if (name == "variableDeclaration")
    {
      this.allowStatements = this.allowFunctionDeclarations = false;
      this.allowVariableDeclarations = true;
    }
    else
  ...
  matches(node)
  {
    ...
    if (!(this.allowStatements && /Statement$/.test(node.type)) &&
...
class ExpressionPlaceholder
{
  ...
  matches(node)
  {
    if (!node)
      return this.optional;
    return (this.allowExpressions && /Expression$/.test(node.type)) ||
