...
const rewritePatterns = [
  ...
  ["return void expression1;", "expression1; return;"],
  ...
  ["if (expression1, expression2) statement1;", "expression1; if (expression2) statement1;"],
  ["if (expression1, expression2, expression3_repeatable) statement1;", "expression1; if (expression2, expression3) statement1;"],
...
function rewriteNode(node, parent, rewritePatterns, scopeManager)
{
  for (let i = 0; i < rewritePatterns.length; i++)
  {
    ...
    if (pattern.type == "VariableDeclaration" && parent.type == "ForStatement" && parent.init == node)
    {
      // Only accept standalone variable declarations as "proper" matches
      continue;
    }
